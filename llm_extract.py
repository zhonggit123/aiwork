...
- question: 题干，必须是**试卷上印刷的原始内容**（学生看到的题目文字），不要自己编造任务描述；若题干需要上传图片则填 ""。**若题干内容开头带有题号（如 1. 2. 一、(1) （1）等），请去掉题号，只保留题干正文。**
- options: 选项数组，无则 []
...
- question: 题干，必须是**试卷上印刷的原始内容**，不要自己编造任务描述；题干需要上传图片时填 ""。**若题干内容开头带有题号（如 1. 2. 一、(1) （1）等），请去掉题号，只保留题干正文。**
- options: 选项数组，无则 []
- candidates: **仅 listening_response（听后应答）题型使用**，所有候选答案的数组（去掉每条候选项最前面的★等标注符号），如 ["In the zoo.", "Mr Smith."]
- answer: 单题答案，**只填选项字母**（如 "A"）；多空时用 blanks
- blanks: 该题多空时使用 [{question, answer, options}]，question 必须是**试卷上印刷的原始内容**（如信息转述起始句），若开头有题号也请去掉；options 为该小题选项数组（无则 []）；**不要**把选项放到顶层 options 字段
