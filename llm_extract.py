...
def _sanitize_english_punctuation(text: Any) -> Any:
    ...
    result = (text
              ...
              .replace("\u2014", "-")
              .replace("\uff5e", "~"))
    # 标点后补英文空格：句号/逗号/问号/感叹号/分号后接非空白字符时补空格
    # 句号：仅在后接字母时补（避免影响小数点 3.14）
    result = re.sub(r'\.(?=[A-Za-z])', '. ', result)
    # 逗号/问号/感叹号/分号：后接非空白字符时补空格
    result = re.sub(r',(?=\S)', ', ', result)
    result = re.sub(r'([?!])(?=\S)', r'\1 ', result)
    result = re.sub(r';(?=\S)', '; ', result)
    # 冒号：后接字母时补空格（避免影响时间格式 3:30）
    result = re.sub(r':(?=[A-Za-z])', ': ', result)
    return result
...
def _repair_json(text: str) -> str:
    """修复 LLM 常见 JSON 错误：
    1. 字符串内裸换行/制表符 → 转义形式
    2. 字符串内未转义的双引号 → 转义（通过前瞻判断是否是合法字符串结束符）
    """
    chars = list(text)
    n = len(chars)
    result = []
...
- 标点：英文内容用半角 ASCII 标点（, . ? ! : ;），标点符号后加英文空格（如 "Hello, world" "I'm fine. She..."），小数点/时间冒号除外（3.14、3:30）；中文内容保持原格式
...
- retell_start: 仅「转述开头」框存在时填对应的转述提示语（如 "The passage is about..."）；无则 ""
...
【信息转述及询问】这是一个大题包含两节，需要输出**两条 JSON**：
- 第一节（信息转述）：type = "listening_fill_and_retell"，按上面的规则输出，有听力原文
- 第二节（询问信息）：type = "listening_response"，这是**独立的一条 JSON**，规则如下：
  - question 填题目说明（如"请根据以下提示提两个问题。每个问题有15秒钟的准备时间和8秒钟的提问时间。"）
  - listening_script 填 ""（空字符串），这节没有听力原文！音频由前端根据 question 自动合成
  - blanks 按小题数输出，每个 blank 包含：
    - question: 中文问题提示（如"你多久去一次足球俱乐部？"）
    - answer: 对应的英文疑问句（如"How often do you go to the football club?"）
  - 设置 is_inquiry: true 标记这是询问信息题
        ...
        for i, s in enumerate(slots[: (n or len(slots))]):
            ...
            if sub > 1:
                ...
                if all_same_structure and first_sq_labels:
                    sq_components = [l for l in ["上传音频", "听力原文", "题干", "转述开头", "参考单词", "设置选项", "设置答案"] if l in first_sq_labels]
