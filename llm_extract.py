...
- question 填页面展示给学生看的题目内容（如单词、音标、短语等）
- keyword 填「参考单词/送评单词」框需要的内容；若材料未提供则填 ""
        ...
        for i, s in enumerate(slots[: (n or len(slots))]):
            ...
            if sub > 1:
                ...
                if all_same_structure and first_sq_labels:
                    ...
                    # 只列出一次小题包含的组件
                    sq_components = [l for l in ["上传音频", "听力原文", "设置选项", "设置答案"] if l in first_sq_labels]
                    if sq_components:
            ...
            if "上传图片" in labels_set and sub <= 1:
