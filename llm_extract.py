...
async def extract_questions_from_word_chunks_async(
    ...
    *,
    ...
    # 若有录题页槽位信息且题数与槽位数一致，按槽位顺序重排题目（纠正 LLM 输出顺序错误）
    slots = (paper_metadata or {}).get("question_slots")
    if slots and isinstance(slots, list) and len(all_questions) == len(slots):
        all_questions = _reorder_questions_by_slots(all_questions, slots)
    return _normalize_questions(all_questions)
...
【通用规则】
- **题目顺序（重要）**：JSON 数组中题目的顺序必须与文档中题目/节的**出现顺序严格一致**。文档中先出现的题必须先输出。例如文档中先有「三、模仿朗读」后有「四、听后记录并转述」，则必须先输出交际朗读（reading_aloud），再输出听后记录并转述（listening_fill_and_retell）；不要按题型或内容重新排序。
...
【通用规则】
- **题目顺序（重要）**：JSON 数组中题目的顺序必须与文档中题目/节的**出现顺序严格一致**。文档中先出现的题必须先输出（如先「三、模仿朗读」后「四、听后记录并转述」则先输出 reading_aloud 再输出 listening_fill_and_retell）；不要按题型重新排序。
